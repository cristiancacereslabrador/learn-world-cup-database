...
# No cambies el código arriba de esta línea. Usa la variable PSQL para hacer consultas a tu base de datos.
...
echo "$($PSQL "SELECT DISTINCT name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT name FROM teams INNER JOIN games ON teams.team_id = games.opponent_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name;")"
